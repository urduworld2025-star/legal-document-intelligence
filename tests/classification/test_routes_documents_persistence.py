...
@pytest.fixture
def client(
    monkeypatch: pytest.MonkeyPatch, db_path: str, stub_classifier_model_dir: Path
) -> TestClient:
    monkeypatch.setattr(settings, "db_path", db_path)
    monkeypatch.setattr(settings, "jwt_secret_key", TEST_JWT_SECRET)
    monkeypatch.setattr(settings, "document_classification_model_dir", str(stub_classifier_model_dir))
    init_db(db_path)
    return TestClient(app)


def test_classify_with_matter_id_persists_result(
    ...
    assert doc["analysis_type"] == "classify"
    assert "classification" in doc["result"]


def test_classify_as_support_staff_returns_403(
    client: TestClient, auth_headers, sample_pdf_path: Path
) -> None:
    with open(sample_pdf_path, "rb") as f:
        response = client.post(
